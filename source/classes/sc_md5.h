...
void sc_md5::update(const unsigned char *buf, int len)
{
  ...
  if((mI[0] + ((uint32_t)len << 3)) < mI[0]) mI[1]++;
  mI[0] += ((uint32_t)len << 3);
  ...
  while(len--)
  {
    ...
    if(mdi == 0x40)
    {
      for(i = 0, ii = 0; i < 16; i++, ii += 4)
        in[i] = (((uint32_t)mIn[ii + 3]) << 24) | (((uint32_t)mIn[ii + 2]) << 16) | (((uint32_t)mIn[ii + 1]) << 8) |
                ((uint32_t)mIn[ii]);
      transform(mBuf, in);
      mdi = 0;
    }
...
void sc_md5::finish()
{
  unsigned char PADDING[64] = {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
